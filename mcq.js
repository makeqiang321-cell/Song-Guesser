// Song Guesser — multiple-choice music quiz (songspot-style: hear a clip, pick A/B/C/D).
// Same shell as game.js; answer UI is 4 option buttons instead of a search box.
(function(){
  ...
  var ctx = null, gain = null, pool = [], all = [], bufCache = {}, cur = null, opts = [], VOLUME = 1.0, playTimer = 0, activeSource = null, progressRAF = 0;

  var el = {
    playKey:$('#playKey'), skip:$('#skipBtn'), options:$('#mcqOptions'),
  ...
  function renderOptions(){
    if(!cur) return;
    var decoys = shuffle(all.filter(function(t){ return t.id !== cur.track.id; })).slice(0, 3);
    var choices = shuffle([cur.track].concat(decoys));
    var letters = ['A','B','C','D'];
    opts = choices.map(function(t, i){ return {track:t, letter:letters[i]}; });
    el.options.innerHTML = '';
    opts.forEach(function(o){
      var b = document.createElement('button');
      b.type = 'button'; b.className = 'songspot-mcq-option';
      b.innerHTML = '<span class="k"></span><span class="t"></span><span class="a"></span>';
      b.querySelector('.k').textContent = o.letter;
      b.querySelector('.t').textContent = o.track.title;
      b.querySelector('.a').textContent = o.track.artist;
  ...
  function markCorrect(){
    opts.forEach(function(o){ if(o.track.id === cur.track.id) o.btn.classList.add('is-correct'); });
  }

  function pick(o, btn){
    if(!cur || cur.done) return;
    if(o.track.id === cur.track.id){
      cur.done = true;
  ...
  function newRound(){
    var go = function(){
      ...
      ensureBuffer(cur.track).catch(function(){});
      cancelAnimationFrame(progressRAF);
  ...
  // Apply initial filter from window.SONG_INITIAL or ?genre/?artist/?decade before the first pool loads.
  (function(){
